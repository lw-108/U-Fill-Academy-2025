...
export const Navbar = ({ children, className }: NavbarProps) => {
    ...
    // Ensure we're on the client before using Framer Motion hooks
    useEffect(() => {
        setIsClient(true);
    }, []);

    // Only use scroll hooks on client side
    const { scrollY } = isClient ? useScroll({
        target: ref,
    ...
    return (
        <motion.div
            ref={ref}
            className={cn("sticky inset-x-0 top-0 z-40 w-full", className)} // Changed top-20 to top-0
        >
